...
namespace TrangChu
{
    public partial class DichVu : Form
    {
                ...
                if (!string.IsNullOrWhiteSpace(defaultMaLich))
                {
                    if (cbxLichDat.DataSource != null && cbxLichDat.DataSource is List<DAL.LichDat> lichDatList)
                    {
                        ...
                        if (selectedLich != null)
                        {
                            ...
                            MessageBox.Show($"⚠️ Không tìm thấy lịch đặt [{defaultMaLich}] trong danh sách!",  "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            cbxLichDat.Enabled = false;
                        }
                    }else{
                        cbxLichDat.Enabled = false;
        ...
        private void SetupEventHandlers()
        {
                ...
                txtTimKiemDVu.TextChanged += TxtTimKiemDVu_TextChanged;                
        }
        ...
        private void LoadLichDat()
        {
                ...
                List<DAL.LichDat> lichDatList = allLichDat.Where(l => l.NgayDat.HasValue && l.NgayDat.Value.Date >= homNay && l.TrangThai == "Đã đặt").OrderBy(l => l.NgayDat).ThenBy(l => l.GioBD) .ToList();
        ...
        private void btnThanhToan_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("❌ Thông tin khách hàng không đầy đủ!","Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
